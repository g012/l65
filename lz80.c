...
static const struct luaL_Reg lz80lib[] = {
    {"image", open_image},
    {NULL, NULL},
};
static int luaopen_lz80(lua_State *L)
{
    luaL_newlib(L, lz80lib);
    return 1;
}

#define SRC_LUA(name) { #name, 0, script_ ## name ## _lua, sizeof(script_ ## name ## _lua) }
#define SRC_LZ80(name) { #name, 1, script_ ## name ## _lz80, sizeof(script_ ## name ## _lz80) }
...
int main(int argc, char *argv[])
{
    ...
    luaL_requiref(L, "lz80", luaopen_lz80, 1); lua_pop(L, 1);
    ...
    luaL_loadbufferx(L, script_z80_lua, sizeof(script_z80_lua), "z80.lua", "b");
    lua_setfield(L, -2, "z80");
    lua_pop(L, 1);

    // error handler
    lua_pushcfunction(L, msghandler);
    // l65.lua script
    luaL_loadbufferx(L, script_lz80_lua, sizeof(script_lz80_lua), "lz80.lua", "b");
    // arg[] table
